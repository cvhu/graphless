class MnodesController < ApplicationController
	...
	def create
		...
		if key_public.nil? or app_id.nil? or model_id.nil? or parent_id.nil?
			obj[:message] = 'All key_public, app_id, model_id, and parent_id are needed.'
		else
			...
			if user.nil? or app.nil? or model.nil?
				...
				mnode = Mnode.create(app_id: app.id, model_id: model.id, parent_id: parent_id)
				if mnode.save
					obj[:status] = 'success'
